<?php
class Fire_fight_model extends CI_Model
{
	...
	{
		if ($table == "fire_fighting") {
			$user = $this->session->userid;
			$role = $this->session->role;
			if ($role == "admin" || $role == "staff") {
				$this->db->select('fire_fighting2.*');
				$this->db->from('fire_fighting2');
	...
	function get_insertid2($table, $id)
	{
		$this->db->select_max($id);
		$hasil = $this->db->get($table);
		return $hasil->row()->$id;
	...
	{
		$this->db->where('ref_id', $id);
		$this->db->where('certificate_type', 'fire_fighting');
	...
	{
		$this->db->select('fire_fighting2.*,DATE_FORMAT(certificate_date, "%d/%m/%Y") as ncertificate_date');
		$this->db->from('fire_fighting2');
