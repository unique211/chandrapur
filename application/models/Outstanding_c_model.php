<?php
class Outstanding_c_model extends CI_Model
{
	...
	{
		if ($table == "outstanding_certificate") {
			$user = $this->session->userid;
			$role = $this->session->role;
			if ($role == "admin" || $role == "staff") {
				$this->db->select('outstanding_certificate.*');
	...
	function showdata_filtered($table, $user)
	{
		//$user = $this->session->userid;
		//	$role = $this->session->role;
		if ($table == "construction_certificate") {
			if ($user == "All") {
				$this->db->select('outstanding_certificate.*');
				$this->db->from('outstanding_certificate');
				$this->db->where('record_status', '1');
	...
	{
		$this->db->where('ref_id', $id);
		$this->db->where('certificate_type', 'outstanding');
