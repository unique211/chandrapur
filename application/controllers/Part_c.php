...
class Part_c extends CI_Controller
{
	...
	public function selectmaxid()
	{
		...
		$data = $this->Part_c_model->get_insertid($table_name, $id);
	...
	public function showdata_filtered()
	{
		...
		$data1 = $this->Part_c_model->showdata_filtered($table_name, $user);
	...
	public function adddata()
	{
		...
		if ($table_name == "part_certificate") {
		...
		if ($id == "") {
			$data1 = $this->Part_c_model->insertdata($data, $table_name);
		} else {
			$data1 = $this->Part_c_model->updatedata($data, $table_name, $id);
			$this->Part_c_model->delete_data2($id);
		}
		echo json_encode($data1);
	}
	...
	public function showdata()
	{
		...
		$data1 = $this->Part_c_model->showalldata($table_name);
	...
	public function deletedata()
	{
		...
		$data1 = $this->Part_c_model->delete_data($table_name, $id);
		$this->Part_c_model->delete_data2($id);
		echo json_encode($data1);
	}
	public function add_doc()
	{
		...
		$data1 = $this->Part_c_model->insertdata2($data, $table_name);
	...
	public function show_doc_id()
	{
		$id     = $this->input->post('id');
		$certificate_type = $this->input->post('certificate_type');
		$data1 = $this->Part_c_model->show_doc_id($certificate_type, $id);
		echo json_encode($data1);
	}
	public function doc_image_upload()
	{
		...
		if ($_FILES['attachment']['size'] > 0) {
			$this->upload->initialize(array(
				"upload_path" => './assets/images/part/',
				"overwrite" => FALSE,
				"max_filename" => 300,
				"remove_spaces" => TRUE,
				"allowed_types" => '*',
	...
	public function get_unique()
	{
		...
		$id = $this->input->post('id');
		// $id="2";
		$data1 = $this->Part_c_model->get_unique($table_name, $id);
	...
	public function doc_image_upload2()
	{
		...
		if (!file_exists('./assets/images/certificate/part/' . $id)) {
			//  mkdir('./assets/images/birth_doc/directory2', 0777, true);
			mkdir('./assets/images/certificate/part/' . $id, 0777, TRUE);
		}
		if ($_FILES['attachment2']['size'] > 0) {
			$this->upload->initialize(array(
				"upload_path" => './assets/images/certificate/part/' . $id . '/',
				"overwrite" => FALSE,
				"max_filename" => 300,
				"remove_spaces" => TRUE,
				"allowed_types" => '*',
	...
	public function download($id)
	{
		...
		//  $args=expload("####", $string);
		//echo $id;
		$get_file = $this->Part_c_model->get_file($id);
		foreach ($get_file as $row) {
			$fileName = $row->upload_doc;
			$unique_no = $row->unique_no;
		}
		if ($fileName) {
			$file = realpath("assets/images/certificate/part/" . $unique_no . "/" . $fileName);
			// check file exists
			if (file_exists($file)) {
				// get file content
