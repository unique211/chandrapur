...
class Property extends CI_Controller
{
	...
	public function selectmaxid()
	{
		...
		$data = $this->Property_model->get_insertid($table_name, $id);
	...
	public function adddata()
	{
		...
		$data1 = "";
		$data = "";
		if ($table_name == "property_transfer_record") {
		...
		if ($id == "") {
			$data1 = $this->Property_model->insertdata($data, $table_name);
		} else {
			$data1 = $this->Property_model->updatedata($data, $table_name, $id);
			$this->Property_model->delete_data2($id);
		}
	...
	public function add_doc()
	{
		...
		$data1 = $this->Property_model->insertdata2($data, $table_name);
		echo json_encode($data1);
	}
	...
	{
		$table_name     = $this->input->post('table_name');
		$data1 = $this->Property_model->showalldata($table_name);
		echo json_encode($data1);
	}
	...
	public function showdata_filtered()
	{
		...
		$data1 = $this->Property_model->showdata_filtered($table_name, $user);
		echo json_encode($data1);
	}
	...
	public function get_unique()
	{
		...
		$id = $this->input->post('id');
		// $id="2";
		$data1 = $this->Property_model->get_unique($table_name, $id);
	...
	public function show_doc_id()
	{
		$id     = $this->input->post('id');
		$certificate_type     = $this->input->post('certificate_type');
		$data1 = $this->Property_model->show_doc_id($certificate_type, $id);
		echo json_encode($data1);
	}
	public function deletedata()
	{
		$table_name     = $this->input->post('table_name');
		$id = $this->input->post('id');
		$data1 = $this->Property_model->delete_data($table_name, $id);
		$this->Property_model->delete_data2($id);
		echo json_encode($data1);
	}
	public function doc_image_upload()
	{
		...
		if ($_FILES['attachment']['size'] > 0) {
			$this->upload->initialize(array(
				"upload_path" => './assets/images/property_record/',
				"overwrite" => FALSE,
				"max_filename" => 300,
				"remove_spaces" => TRUE,
				"allowed_types" => '*',
	...
	public function doc_image_upload2()
	{
		...
		if (!file_exists('./assets/images/certificate/property_record/' . $id)) {
			//  mkdir('./assets/images/birth_doc/directory2', 0777, true);
			mkdir('./assets/images/certificate/property_record/' . $id, 0777, TRUE);
		}
		if ($_FILES['attachment2']['size'] > 0) {
			$this->upload->initialize(array(
				"upload_path" => './assets/images/certificate/property_record/' . $id . '/',
				"overwrite" => FALSE,
				"max_filename" => 300,
				"remove_spaces" => TRUE,
				"allowed_types" => '*',
	...
	public function download($id)
	{
		...
		//  $args=expload("####", $string);
		//echo $id;
		$get_file = $this->Property_model->get_file($id);
		foreach ($get_file as $row) {
			$fileName = $row->upload_doc;
			$unique_no = $row->unique_no;
		}
		if ($fileName) {
			$file = realpath("assets/images/certificate/property_record/" . $unique_no . "/" . $fileName);
			// check file exists
			if (file_exists($file)) {
				// get file content
