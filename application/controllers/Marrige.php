...
class Marrige extends CI_Controller
{
	...
	public function adddata()
	{
		...
		if ($id == "" &&  $table_name == 'marrige_receipt') {
	...
	public function cash_counter_reciept_voucher()
	{

		$where = $this->input->post('print_vouchar');
		//$where=1;
		$data['record'] = $this->m->get_voucher($where);
		$this->load->view('cash_counter_reciept_voucher', $data);
	}
	...
	public function showdata()
	{
		...
		if ($mode == "1") {
			$data1 = $this->m->showalldata($table_name, $frm, $to, $user);
		} else {
			$data1 = $this->m->showalldatareceiptdt($table_name, $frm, $to, $user);
		}
	...
	public function showDatewiseReceipt()
	{
		...
		$get_receipt_id = $this->m->get_receipt_id($date);
		$data['record'] = $this->m->showDatewiseReceipt($date);
		$data['record_extra'] = $this->m->showDatewiseReceiptExtra($date);
	...
	{
		$year = $this->input->post('year');
		$data = $this->m->get_receipt_id2($year);
		echo json_encode($data);
	}
	public function showdatavoucher()
	{
		$data = $this->m->getAllVoucher();
		echo json_encode($data);
	}
}
