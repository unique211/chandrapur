$(document).ready(function() {
    var table_name = "municipal_election";
    $(document).on("submit", "#master_form", function(e) {
        ...
        var date1 = $('#date').val();
        var news_desc = $('#news').val();
        var event_desc = $('#event').val();
        ...
        $.ajax({
            type: "POST",
            url: baseurl + "Muni_Election/adddata",
            data: {
                id: id,
                parent: parent,
                news_desc: news_desc,
    ...
    function datashow() {
        $.ajax({
            type: "POST",
            url: baseurl + "Muni_Election/showdata",
            data: {
                table_name: table_name,
            },
            dataType: "JSON",
            ...
            success: function(data) {
                ...
                html += '<table id="myTable" class="table table-striped">' +
                    ...
                    '<th><font style="font-weight:bold">News Descrption</font></th>' +
                    '<th ><font style="font-weight:bold">घडामोडीचे वर्णन</font></th>' +
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    html += '<tr>' +
                        ...
                        '<td id="news_desc_' + data[i].id + '">' + data[i].news_desc + '</td>' +
                        '<td id="event_desc_' + data[i].id + '">' + data[i].event_desc + '</td>' +
        ...
        if (id1 != "") {
            swal({
                ...
                function() {
                    $.ajax({
                        type: "POST",
                        url: baseurl + "Muni_Election/deletedata",
                        dataType: "JSON",
                        data: {
                            id: id1,
                            table_name: table_name,
    ...
    $('#attachment').ajaxfileupload({
        'action': baseurl + 'Muni_Election/doc_image_upload',
