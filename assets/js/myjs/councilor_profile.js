$(document).ready(function() {
    var table_name = "councilor_profile";
    $(document).on("submit", "#master_form", function(e) {
        ...
        var ward = $('#ward').val();
        var ward_m = $('#ward_m').val();

        $.ajax({
            type: "POST",
            url: baseurl + "Councilor_profile/adddata",
            data: {
                id: id,
                file: file,
                name: name,
    ...
    function datashow() {
        $.ajax({
            type: "POST",
            url: baseurl + "Councilor_profile/showdata",
            data: {
                table_name: table_name,
            },
            dataType: "JSON",
            ...
            success: function(data) {
                ...
                html += '<table id="myTable" class="table table-striped">' +
                    ...
                    '<th ><font style="font-weight:bold">पद</font></th>' +
                    '<th ><font style="font-weight:bold">वर्णन</font></th>' +
                    '<th ><font style="font-weight:bold">वार्ड (मराठी)</font></th>' +
                    '<th ><font style="font-weight:bold">Ward (English)</font></th>' +
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    html += '<tr>' +
                        ...
                        '<td  id="ward_m_' + data[i].id + '">' + data[i].ward_m + '</td>' +
                        '<td  id="ward_' + data[i].id + '">' + data[i].ward + '</td>' +
        ...
        if (id1 != "") {
            swal({
                ...
                function() {
                    $.ajax({
                        type: "POST",
                        url: baseurl + "Councilor_profile/deletedata",
                        dataType: "JSON",
                        data: {
                            id: id1,
                            table_name: table_name,
    ...
    $(document).on("click", ".edit_data", function() {
        ...
        var ward = $('#ward_' + id).html();
        var ward_m = $('#ward_m_' + id).html();
    ...
    $('#attachment').ajaxfileupload({
        'action': baseurl + 'Councilor_profile/doc_image_upload',
