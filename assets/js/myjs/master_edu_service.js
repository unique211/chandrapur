$(document).ready(function() {
    var table_name = "master_edu_service";
    $(document).on("submit", "#master_form", function(e) {
        ...
        var service = $('#title').val();
        var service_m = $('#title_m').val();
        $.ajax({
            type: "POST",
            url: baseurl + "Master_edu_service/adddata",
            data: {
                id: id,
                desc: desc,
                desc_m: desc_m,
    ...
    function datashow() {
        $.ajax({
            type: "POST",
            url: baseurl + "Master_edu_service/showdata",
            data: {
                table_name: table_name,
            },
            dataType: "JSON",
            ...
            success: function(data) {
                ...
                html += '<table id="myTable" class="table table-striped">' +
                    ...
                    '<th ><font style="font-weight:bold">सेवा</font></th>' +
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    html += '<tr>' +
                        ...
                        '<td  id="title_' + data[i].id + '">' + data[i].service + '</td>' +
                        '<td  id="title_m_' + data[i].id + '">' + data[i].service_m + '</td>' +
        ...
        if (id1 != "") {
            swal({
                ...
                function() {
                    $.ajax({
                        type: "POST",
                        url: baseurl + "Master_edu_service/deletedata",
                        dataType: "JSON",
                        data: {
                            id: id1,
                            table_name: table_name,
    ...
    $('#attachment').ajaxfileupload({
        'action': baseurl + 'Master_edu_service/doc_image_upload',
