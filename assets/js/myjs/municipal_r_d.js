$(document).ready(function() {
    var table_name = "municipal_resolutions_decisions";
    $(document).on("submit", "#master_form", function(e) {
        ...
        var date1 = $('#date').val();
        var decisions = $('#news').val();
        var decisions_m = $('#event').val();
        var file = $('#file_attachother').val();
        var resolutions = $('#title').val();
        var resolutions_m = $('#title_m').val();

        var dateslt = date1.split('/');
        var date = dateslt[2] + '-' + dateslt[1] + '-' + dateslt[0];
        $.ajax({
            type: "POST",
            url: baseurl + "Municipal_r_d/adddata",
            data: {
                id: id,
                decisions: decisions,
                decisions_m: decisions_m,
                file: file,
    ...
    function datashow() {
        $.ajax({
            type: "POST",
            url: baseurl + "Municipal_r_d/showdata",
            data: {
                table_name: table_name,
            },
            dataType: "JSON",
            ...
            success: function(data) {
                ...
                html += '<table id="myTable" class="table table-striped">' +
                    ...
                    '<th ><font style="font-weight:bold">Resolution (English)</font></th>' +
                    '<th ><font style="font-weight:bold">ठराव(मराठी)</font></th>' +
                    '<th ><font style="font-weight:bold">Decision (English)</font></th>' +
                    '<th ><font style="font-weight:bold">निर्णय(मराठी)</font></th>' +
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    html += '<tr>' +
                        ...
                        '<td  id="title_' + data[i].id + '">' + data[i].resolutions + '</td>' +
                        '<td  id="title_m_' + data[i].id + '">' + data[i].resolutions_m + '</td>' +
                        '<td  id="news_desc_' + data[i].id + '">' + data[i].decisions + '</td>' +
                        '<td  id="event_desc_' + data[i].id + '">' + data[i].decisions_m + '</td>' +
        ...
        if (id1 != "") {
            swal({
                ...
                function() {
                    $.ajax({
                        type: "POST",
                        url: baseurl + "Municipal_r_d/deletedata",
                        dataType: "JSON",
                        data: {
                            id: id1,
                            table_name: table_name,
    ...
    $('#attachment').ajaxfileupload({
        'action': baseurl + 'Municipal_r_d/doc_image_upload',
