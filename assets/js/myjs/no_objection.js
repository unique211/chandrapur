$(document).ready(function() {
            ...
            success: function(data) {
                ...
                if (r != 0 || r != "0") {
                    for (var i = 0; i < r; i++) {
                        ...
                        var certificate_type = "no_objection";
                        //  table_name = "doc_upload";

                        //  alert(ref_id);
                        $.ajax({
                            type: "POST",
                            url: baseurl + "Noc/add_doc",
    ...
    function datashow() {
        ...
        if (role == "admin" || role == "staff") {
            ...
            $.ajax({
                type: "POST",
                url: baseurl + "Noc/showdata_filtered",
                data: {
                    table_name: table_name,
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        html += '<tr>' +
                            ...
                            '<td id="download_' + data[i].id + '"><a href=' + baseurl + 'Noc/download/' + data[i].id + '>' + download + '</a></td>' +
    ...
    $(document).on("click", ".edit_data", function() {
        ...
        var house_no = $('#house_no_' + id).html();
    ...
    $('#attachment').ajaxfileupload({
        'action': baseurl + 'Noc/doc_image_upload',
    ...
    $(document).on('submit', '#reject_form', function(e) {
        ...
        $.ajax({
            type: "POST",
            url: baseurl + "Noc/update_status2",
    ...
    $('#attachment2').ajaxfileupload({
        'action': baseurl + 'Noc/doc_image_upload2',
